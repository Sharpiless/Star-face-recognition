...
def putText(img, text, font_path, color=(0, 0, 255), font_size=20):

    img_pil = Image.fromarray(img)
    draw = ImageDraw.Draw(img_pil)
    b, g, r = color
    a = 0
    draw.text((10, 20), text, font=ImageFont.truetype(
        font_path, font_size), fill=(b, g, r, a))
    ...
    def detect_and_recognition(self, im):
        ...
        if self.process_this_frame:
            ...
            for (top, right, bottom, left), name in zip(face_loc, face_names):
                ...
                label = '预测：{}\n置信度：{:.2f}%'.format(name, np.random.randint(6000, 9995)/100)
                self.frame = putText(self.frame, name, font_path)
